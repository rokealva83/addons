...
class WebsiteSearch(http.Controller):
    ...
    def search_results(self, page=1, sorting='date', search='', **post):
        ...
        if len(search)<self._min_search_len:
            return request.website.render("theme_aditim.search_page", None)
        ...
        if search_on_products:
            ...
            if sql_query:
                sql_query+=' UNION ALL '
            sql_query+="""
                  SELECT DISTINCT 'Product' as result_type, pd.id as result_id, pd.name as result_name,  'website_sale' as template_module, pd.website_description as template_source, pd.website_meta_description, pd.website_meta_title, pd.website_meta_keywords, '/page/product_interior_page/p/' as result_path, '' as result_image, '' as result_lang, '' as result_lang_text, pd.write_date as result_date
        ...
        # Render results
        return request.website.render("theme_aditim.search_page", values)

    @http.route(['/bsearch_results',
                 '/bsearch_results/page/<int:page>'
                 ], type='http', auth="public", website=True)
